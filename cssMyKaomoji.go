...
func makeDemoFile(kl map[string]string) {
	css := "<!DOCTYPE html><html><head><meta charset=\"utf-8\"><link rel='stylesheet' type='text/css' href='../kaomoji.css'><style>body{display:grid;grid-template-columns:repeat(4,1fr);grid-gap:.5rem;}span{padding:1rem;border:2px dashed lightblue;text-align:center}sup{padding-top: .5em;margin-top: .5em;border-top: 1px dashed lightblue;display:block}::before{font-weight: bold;font-size: 1.5em;}</style></head><body>"
